...
// ── CAPTURA AUTOMÁTICA DEL TOKEN ─────────────────────────────────────────
function interceptToken(){
  const origOpen=window.XMLHttpRequest.prototype.open;
  window.XMLHttpRequest.prototype.open=function(m,url){
    if(url.includes('/api/')){
      const origSet=this.setRequestHeader.bind(this);
      this.setRequestHeader=function(name,value){
        if(name==='Authorization'&&value&&value.startsWith('eyJ')){
          AUTH_TOKEN=value;
        }
  ...
  window.fetch=function(url,opts){
    if(typeof url==='string'&&url.includes('/api/')&&opts&&opts.headers){
      ...
      if(auth&&auth.startsWith('eyJ')) AUTH_TOKEN=auth;
    }
...
async function loadLeads(){
  ...
  // Si no tenemos token aún, esperar hasta 3 segundos
  if(!AUTH_TOKEN){
    setStatus('Esperando token de sesión...','warn');
    for(let i=0;i<6;i++){
      await sleep(500);
      if(AUTH_TOKEN) break;
    }
  ...
  if(!AUTH_TOKEN){
    setStatus('⚠️ Recargá el listado de leads en ASOFIX para capturar el token','warn');
    return;
  }
  try{
